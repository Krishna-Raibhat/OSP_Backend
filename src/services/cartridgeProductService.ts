...
export async function createCartridgeProduct(input: {
  brand_id: string;
  category_id: string;
  product_name: string;
  model_number: string;
  ...
  if (!brand_id || !category_id || !product_name || !model_number || unit_price === undefined) {
    throw new HttpError(400, "brand_id, category_id, product_name, model_number, and unit_price are required.");
  }
  ...
  try {
    const q = `
      ...
      RETURNING *;
    `;
    const values = [brand_id, category_id, product_name.trim(), model_number.trim(), description, unit_price, special_price, is_active, created_by];
    const result = await pool.query<CartridgeProduct>(q, values);
    return result.rows[0];
  } catch (err: any) {
    if (isPgUniqueViolation(err)) {
      throw new HttpError(409, "Product with this model number already exists.");
    }
    throw err;
  }
}
...
export async function getAllCartridgeProducts() {
  const q = `SELECT * FROM cartridge_products ORDER BY created_at DESC;`;
  const result = await pool.query<CartridgeProduct>(q);
  return result.rows;
}

export async function getCartridgeProductById(id: string) {
  const q = `SELECT * FROM cartridge_products WHERE id = $1;`;
...
export async function deleteCartridgeProduct(id: string) {
  try {
    ...
    if (isPgForeignKeyViolation(err)) {
      throw new HttpError(400, "Cannot delete product with associated inventory or orders.");
    }
    throw err;
  }
}
