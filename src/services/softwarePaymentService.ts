...
import type { SoftwarePayment } from "../models/softwareModels";
import * as orderService from "./softwareOrderService";
...
export async function createCODPayment(order_id: string) {
  ...
  const orderQuery = `SELECT * FROM software_orders WHERE id = $1 AND status = 'pending';`;
  ...
  const result = await pool.query<SoftwarePayment>(paymentQuery, [order_id, order.total]);
...
export async function confirmCODPayment(input: {
  ...
  try {
    ...
    // Get payment
    const paymentQuery = `SELECT * FROM software_payments WHERE id = $1 AND payment_type = 'cod';`;
    const paymentResult = await client.query<SoftwarePayment>(paymentQuery, [payment_id]);
    ...
    await client.query(
      `UPDATE software_orders SET status = 'paid', updated_at = NOW() WHERE id = $1`,
      [payment.software_order_id]
    );

    // Generate licenses
    await orderService.generateLicenses(payment.software_order_id);

    await client.query("COMMIT");

    return { message: "COD payment confirmed and licenses generated." };
  } catch (error) {
...
export async function getPaymentByOrderId(order_id: string) {
  const q = `SELECT * FROM software_payments WHERE software_order_id = $1;`;
  const result = await pool.query<SoftwarePayment>(q, [order_id]);
...
export async function getPendingCODPayments() {
  const q = `
    ...
    FROM software_payments p
    JOIN software_orders o ON p.software_order_id = o.id
...
export async function createManualPayment(input: {
  ...
  try {
    ...
    // Verify order exists
    const orderQuery = `SELECT * FROM software_orders WHERE id = $1;`;
    ...
    const paymentResult = await client.query<SoftwarePayment>(paymentQuery, [
      order_id,
      manual_reference,
      amount,
    ]);
    ...
    // Generate licenses
    await orderService.generateLicenses(order_id);
