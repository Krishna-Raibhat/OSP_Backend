...
export async function generateQrCode( res: Response, product: CartridgeProduct) {
    try {      const data = await qrService.generateProductQR(product);
      return res.status(201).json({ message: "QR code generated successfully.", data });
    } catch (err: any) {
...
export async function deactivateQrCode(req: Request, res: Response) {
    try {
      ...
      return res.status(200).json({ message: "QR code deactivated successfully." });
    } catch (err: any) {
      if (err instanceof HttpError)
        return res.status(err.status).json({ message: err.message });
...
export async function reactivateQrCode(req: Request, res: Response) {
    try {
      ...
      return res.status(200).json({ message: "QR code reactivated successfully." });
    } catch (err: any) {
      if (err instanceof HttpError)
        return res.status(err.status).json({ message: err.message });
...
export async function updateQrCode( res: Response, product: CartridgeProduct) {
    try {
      ...
      return res.status(200).json({ message: "QR code updated successfully.", data });
    } catch (err: any) {
      if (err instanceof HttpError)
        return res.status(err.status).json({ message: err.message });
